...
In the Mr. Trash Wheel data set there are `r nrow(mr_df)` rows and `r ncol(mr_df)` variables. Some of the key variables in this dataset are Weight of trash in tons and the volume of trash in cubic yards. The total number of sports balls collected in 2020 by Mr. Trash Wheel is `r mr_df %>% filter(year == 2020) %>% summarize(sum(total))`
